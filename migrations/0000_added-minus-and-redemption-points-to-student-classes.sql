...
--> statement-breakpoint
CREATE UNIQUE INDEX `emailUniqueIndex` ON `beta_signups` (lower("email"));--> statement-breakpoint
...
CREATE TABLE `student_sub_groups` (
	`enrollment_id` text PRIMARY KEY NOT NULL,
	`sub_group_id` text NOT NULL,
	`student_id` text NOT NULL,
	`enrollment_date` text DEFAULT CURRENT_TIMESTAMP NOT NULL,
	FOREIGN KEY (`sub_group_id`) REFERENCES `sub_groups`(`sub_group_id`) ON UPDATE no action ON DELETE no action,
	FOREIGN KEY (`student_id`) REFERENCES `students`(`student_id`) ON UPDATE no action ON DELETE no action
);
--> statement-breakpoint
CREATE INDEX `sub_groups_by_student_id_idx` ON `student_sub_groups` (`student_id`);--> statement-breakpoint
...
--> statement-breakpoint
CREATE INDEX `sub_groups_by_class_id_idx` ON `sub_groups` (`class_id`);--> statement-breakpoint
CREATE INDEX `sub_groups_by_group_id_idx` ON `sub_groups` (`group_id`);--> statement-breakpoint
